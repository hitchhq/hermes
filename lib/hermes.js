...
proto.use = function use () {
  ...
  const is_router = callbacks.length === 1 && _isRouter(callbacks[0]);

  if (path && is_router) {
    _useRouter.call(this, path, callbacks[0], 'all');
  } else if (path && callbacks.length) {
    _use.call(this, path, callbacks);
  } else if (!path && is_router) {
    _useRouter.call(this, '', callbacks[0], 'all');
  } else if (!path && callbacks) {
    _use.call(this, '*', callbacks);
  }
...
function _isRouter (fn) {
  return fn && fn.constructor && fn.constructor.name === 'HermesRouter';
}
...
proto.handle = function handle (message, out) {
  ...
  function next (err) {
    ...
    if (err) {
      call(layer.handle, null, err, null, next);
      return;
    }
  ...
  next(message instanceof Error ? message : undefined);
};
